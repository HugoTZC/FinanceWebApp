...
class SupabaseRequestError(RuntimeError):
    """Raised when the Data API rejects or cannot complete a request."""
...
class SupabaseRestClient:
    """Call PostgREST without exposing the privileged API key to clients."""
    ...
    def __init__(
        ...
        *,
        ...
        # Modern sb_secret keys are opaque and should be sent on `apikey` only.
        # Keep Authorization for the legacy service_role JWT during the overlap.
    ...
    def headers(self) -> httpx.Headers:
        """Expose header names for tests without logging their values."""
    ...
    def select(
        ...
        if not isinstance(payload, list):
            raise SupabaseRequestError(response.status_code, f"select {table}")
        return payload
